...
class LeggedRobot(BaseTask):
    ...
    def create_sim(self):
        ...
        if self.cfg.video_logger.enable_video_logger:
        ...
        start = time()
        print("*" * 80)
        print(colorama.Fore.GREEN + "Start creating ground...")
        if mesh_type in ['heightfield', 'trimesh']:
            self.terrain = Terrain(self.cfg.terrain, self.num_envs) # todo: need to tune terrain
        if mesh_type == 'plane':
        ...
        print(colorama.Fore.GREEN + "Finished creating ground. Time taken {:.2f} s".format(time() - start))
        print("*" * 80)
        self._create_envs()
    ...
    def step(self, actions):
        ...
        clip_actions = self.cfg.normalization.clip_actions / torch.tensor(self.cfg.control.action_scale, dtype=torch.float, device=self.device)  
    ...
    def post_physics_step(self):
        ...
        self.cur_goals = self._gather_cur_goals() # 提取当前目标
        self.next_goals = self._gather_cur_goals(future=1) # 提取下一个目标
        ...
        ## save video
        if self.cfg.video_logger.enable_video_logger:
            if self.video_logger.is_complete() == False:
                sampled_env_id = self.cfg.video_logger.sampled_env_id
                self.gym.render_all_camera_sensors(self.sim)
                bx, by, bz = self.root_states[sampled_env_id, 0], self.root_states[sampled_env_id, 1], \
                    self.root_states[sampled_env_id, 2]
                self.gym.set_camera_location(self.camera_handler, self.envs[self.cfg.video_logger.sampled_env_id],
                                             gymapi.Vec3(bx, by - 1.0, bz + 0.3),
                                             gymapi.Vec3(bx, by, bz - 0.2))
                img = self.gym.get_camera_image(self.sim, self.envs[self.cfg.video_logger.sampled_env_id],
                                                self.camera_handler, gymapi.IMAGE_COLOR)
                img = img.reshape((self.cfg.video_logger.canvas_size[1], self.cfg.video_logger.canvas_size[0], 4))
                self.video_logger.read_single_frame(img)
    ...
    def compute_observations(self):
        ...
        if self.global_counter % 5 == 0:  # step 5次更新一次yaw todo: need to write in config
        ...
        obs_buf = torch.cat((  
            ...
            imu_obs,  # [1,2] 2
            0 * self.delta_yaw[:, None],  # 当前yaw
            self.delta_yaw[:, None],  # 目标yaw
    ...
    def _compute_torques(self, actions):
        ...
        """
        # pd controller
        actions_scaled = actions * torch.tensor(self.cfg.control.action_scale, dtype=torch.float, device=self.device)
    ...
    def _init_buffers(self):
        ...
        actor_root_state = self.gym.acquire_actor_root_state_tensor(self.sim) # base state: pose in R7, vel in R6
        dof_state_tensor = self.gym.acquire_dof_state_tensor(self.sim) # joint state: 12 + vel: 12
        net_contact_forces = self.gym.acquire_net_contact_force_tensor(self.sim) # all link forces
        force_sensor_tensor = self.gym.acquire_force_sensor_tensor(self.sim)  # force sensor
        rigid_body_state_tensor = self.gym.acquire_rigid_body_state_tensor(self.sim) # all link state
        self.gym.refresh_dof_state_tensor(self.sim)
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self.gym.refresh_net_contact_force_tensor(self.sim)
        self.gym.refresh_rigid_body_state_tensor(self.sim)
        ...
        self.rigid_body_states = gymtorch.wrap_tensor(rigid_body_state_tensor).view(self.num_envs, -1, 13) # num_envs, 每个envs的刚体数量，pose in R7, vel in R6
        self.dof_state = gymtorch.wrap_tensor(dof_state_tensor)
        self.dof_pos = self.dof_state.view(self.num_envs, self.num_dof, 2)[..., 0] # ...: 选择张量的所有前面的维度, (num_envs, num_dof)，不包括最后一个维度
        ...
        # 到达每一个goal后的停歇时间，最大不超过1sim 步长
        self.reach_goal_timer = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)

        str_rng = self.cfg.domain_rand.motor_strength_range # 电机limit factor
        # [str_rng[0], str_rng[1]]之间的随机值
        ...
        if self.cfg.env.history_encoding:
            self.obs_history_buf = torch.zeros(self.num_envs, self.cfg.env.history_len, self.cfg.env.n_proprio,
                                               device=self.device, dtype=torch.float) # obs历史数据只装n_proprio
        ...
        self.feet_air_time = torch.zeros(self.num_envs, self.feet_indices.shape[0], dtype=torch.float,
                                         device=self.device, requires_grad=False) # self.feet_indices.shape[0] = 4
        self.last_contacts = torch.zeros(self.num_envs, len(self.feet_indices), dtype=torch.bool, device=self.device,
                                         requires_grad=False) # len(self.feet_indices) = 4
        ...
        # for save video
        if self.cfg.video_logger.enable_video_logger:
            self.video_logger = video_logger(self.cfg.video_logger)
            self.set_logger_camera()


    #################################################################
    ##########################  save video  #########################
    #################################################################
    def set_logger_camera(self): # todo: multi video
        sampled_env_id = self.cfg.video_logger.sampled_env_id
        camera_properties = gymapi.CameraProperties()
        camera_properties.width = self.cfg.video_logger.canvas_size[0]
        camera_properties.height = self.cfg.video_logger.canvas_size[1]
        # enable GPU tensor fetching. (to prevent from copying infor from GPU to CPU)
        # camera_properties.enable_tensors = True
        head_depth_cam = self.gym.create_camera_sensor(self.envs[sampled_env_id], camera_properties)
        bx, by, bz = self.root_states[sampled_env_id, 0], self.root_states[sampled_env_id, 1], \
            self.root_states[sampled_env_id, 2]
        self.gym.set_camera_location(head_depth_cam, self.envs[sampled_env_id],
                                     gymapi.Vec3(bx, by - 1.0, bz + 0.5),
                                     gymapi.Vec3(bx, by, bz))
        self.camera_handler = head_depth_cam

        print(f"camera_handler: {self.camera_handler}")

    def log_video(self,iter, log_dir):
        if iter % self.cfg.video_logger.video_log_interval==0:
            if self.video_logger.is_complete():
                self.video_logger.save_video(iter, log_dir)
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.terrain.measure_heights:
            if self.global_counter % self.cfg.depth.update_interval == 0:  # update_interval个步长更新一次地面
    ...
    def _gather_cur_goals(self, future=0):
        return self.env_goals.gather(1, (self.cur_goal_idx[:, None, None] + future).expand(-1, -1,
                                                                                           self.env_goals.shape[
                                                                                               -1])).squeeze(1)

    def _update_goals(self):  # 更新目标和heading
        next_flag = self.reach_goal_timer > self.cfg.env.reach_goal_delay / self.dt  # 等待的最小时间
    ...
    def _resample_commands(self, env_ids):
        ...
        # set small commands to zero 以vx为标准，忽略vy todo: need to tune
    ...
    def reset_idx(self, env_ids):
        ...
        self._reset_root_states(env_ids, group_wall)
    ...
    def attach_camera(self, i, env_handle, actor_handle):
        ...
        if self.cfg.depth.use_camera:
            ...
            camera_angle = np.random.uniform(config.angle[0], config.angle[1]) # Notes:随机化相机角度！

            local_transform.p = gymapi.Vec3(*camera_position)
            local_transform.r = gymapi.Quat.from_euler_zyx(0, np.radians(camera_angle), 0) # pitch
    ...
    def process_depth_image(self, depth_image, env_id):
        ...
        depth_image += self.cfg.depth.dis_noise * 2 * (torch.rand(1)-0.5)[0] # depth噪声 todo: need to tune
        depth_image = torch.clip(depth_image, -self.cfg.depth.far_clip, -self.cfg.depth.near_clip) # 裁掉过近或过远
    ...
    def reindex_feet(self, vec):
        return vec[:, [2, 0, 3, 1]]  # reindex feet to RF, LF, RH, LH
    ...
    def _parse_cfg(self, cfg):
        ...
        self.reward_scales = class_to_dict(self.cfg.rewards.scales) # reward weight
        reward_norm_factor = 1 # reward weight 归一化
        for rew in self.reward_scales:
            self.reward_scales[rew] = self.reward_scales[rew] / reward_norm_factor
        if self.cfg.commands.curriculum:
            self.command_ranges = class_to_dict(self.cfg.commands.ranges)
        else:
            self.command_ranges = class_to_dict(self.cfg.commands.max_ranges) # 固定cmd范围, 对cmd不进行课程学习
        if self.cfg.terrain.mesh_type not in ['heightfield', 'trimesh']:
            self.cfg.terrain.curriculum = False
        # 最大学习时长and步长
        self.max_episode_length_s = self.cfg.env.episode_length_s
        self.max_episode_length = np.ceil(self.max_episode_length_s / self.dt)
        # push 频率 todo: maybe need to random
    ...
    def _draw_feet(self):
        if hasattr(self, 'feet_at_edge'):
            ...
            edge_geom = gymutil.WireframeSphereGeometry(0.02, 16, 16, None, color=(1, 0, 0)) # red
    ...
    def _create_envs(self):
        ...
        print(colorama.Fore.GREEN + f"self.dof_names: {self.dof_names}")
        print(colorama.Fore.CYAN + f"feet_names: {feet_names}")
        # dof order: ['LF_HAA', 'LF_HFE', 'LF_KFE', 'LF_WHEEL', 
        # 'LH_HAA', 'LH_HFE', 'LH_KFE', 'LH_WHEEL', 
        # 'RF_HAA', 'RF_HFE', 'RF_KFE', 'RF_WHEEL', 
        # 'RH_HAA', 'RH_HFE', 'RH_KFE', 'RH_WHEEL']

        # foot order: ['LF_FOOT', 'LH_FOOT', 'RF_FOOT', 'RH_FOOT']

        for s in ["RF_FOOT", "LF_FOOT", "RH_FOOT", "LH_FOOT"]:
        ...
        self.mass_params_tensor = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device,
                                              requires_grad=False)
        print(colorama.Fore.BLUE + "Creating env...")
        for i in tqdm(range(self.num_envs)):
            # create env instance and random robot
            env_handle = self.gym.create_env(self.sim, env_lower, env_upper, int(np.sqrt(self.num_envs)))
            pos = self.env_origins[i].clone()
            if self.cfg.env.randomize_start_pos:
                pos[:2] += torch_rand_float(-1., 1., (2, 1), device=self.device).squeeze(1) # 只在start x和y上随机
            if self.cfg.env.randomize_start_yaw:
                rand_yaw_quat = gymapi.Quat.from_euler_zyx(0., 0.,
                                                           self.cfg.env.rand_yaw_range * np.random.uniform(-1, 1))
            ...
            anymal_handle = self.gym.create_actor(env_handle, robot_asset, start_pose, "anymal", i,
                                                  self.cfg.asset.self_collisions, 0) # todo: need to rewirte the name
    ...
    ##########################  reward  #############################
    #################################################################
    # exp: 强调较大的值, 奖励随着误差减小而快速增加，用于奖励
    # square: 用于惩罚
    ############ 奖励 ############
    def _reward_tracking_goal_vel(self):
        # r_tracking = min(<v, d>, v_cmd)
        norm = torch.norm(self.target_pos_rel, dim=-1, keepdim=True)
        target_vec_norm = self.target_pos_rel / (norm + 1e-5)
    ...
    def _reward_feet_edge(self):
        # self.cfg.terrain.horizontal_scale 惩罚到edge的距离阈值
